"""JAV Star Names Finder V2.0 GUI Version
...
from PySide6.QtWidgets import QMainWindow, QApplication, QWidget, QFileDialog, QDialog, QTableWidgetItem
from bs4 import BeautifulSoup

from dialog_inform import Ui_Dialog
from dialog_ok_cancel import Ui_Dialog as Ui_DialogOkCancel
from JAV_star_names_finder_main_window import Ui_Form
...
class MainWindow(QWidget, Ui_Form):
    def __init__(self, *args, obj=None, **kwargs):
        ...
        self.dialog_inform = DialogInform()
        self.dialog_ok_cancel = DialogOkCancel()
    ...
    def select_path(self):
        ...
        if path in ("C:/", "C:/Windows", "C:/Program Files (x86)", "C:/Program Files"):
            self.dialog_inform.textEdit.setText("C 드라이브 최상위 경로 또는 Windows 주요 폴더를 선택했습니다. "
                                                + "이러한 시스템 폴더에 대한 작업은 위험하므로 다른 경로를 선택해 주십시오.")
            self.dialog_inform.textEdit.setAlignment(Qt.AlignHCenter)
            self.dialog_inform.exec()
        elif path == "":
    ...
    def change_prev_separator(self):
        ...
        if self.label_work_path.text() == "*아직 선택되지 않음*":
            self.dialog_inform.textEdit.setText("먼저 작업 경로를 선택해 주십시오.")
            self.dialog_inform.textEdit.setAlignment(Qt.AlignHCenter)
            self.dialog_inform.exec()
            return None
        ...
        if (
            ...
            self.dialog_inform.textEdit.setText("기존 출연자 위치 및 구분자가 신규 설정과 동일하므로 작업할 내용이 없습니다.")
            self.dialog_inform.textEdit.setAlignment(Qt.AlignHCenter)
            self.dialog_inform.exec()
            return None

        self.dialog_ok_cancel.textEdit.setText(
            ...
            + "작업을 진행하려면 [확인]을, 취소하려면 [취소]를 선택하십시오."
        )
        if self.dialog_ok_cancel.exec() != QDialog.Accepted:
            return None
    ...
    def copy_to_clipboard(self):
        if self.tableWidget.rowCount() == 0:
            self.dialog_inform.textEdit.setText(
        ...
        if clipboard.mimeData().hasText():
            self.dialog_inform.textEdit.setText(
                "작업 내용이 클립보드로 복사되었습니다. 메모장 또는 엑셀에 붙여넣어 활용할 수 있습니다.")
            self.dialog_inform.textEdit.setAlignment(Qt.AlignHCenter)
            self.dialog_inform.exec()
        else:
            self.dialog_inform.textEdit.setText(
    ...
    def search_stars(self):
        ...
        self.search_thread.progress_message.connect(self.update_progress)
        self.search_thread.start()

    def update_progress(self, progress):
        self.label_status.setText(progress)
        if progress.find("작업 완료") >= 0:
            self.pushButton_3.setEnabled(True)


class DialogInform(QDialog, Ui_Dialog):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
...
class DialogOkCancel(QDialog, Ui_DialogOkCancel):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
...
class SearchThread(QThread):
    progress_message = Signal(str)  # Signal to update progress

    def __init__(self, parent=None):
        super().__init__(parent)

    def run(self):
        main_window.save_current_settings()

        if main_window.request_no > 50:
            main_window.request_no_over_50_inform()
            return None

        if main_window.label_work_path.text() == "*아직 선택되지 않음*":
            main_window.dialog_inform.textEdit.setText("먼저 작업 경로를 선택해 주십시오.")
            main_window.dialog_inform.textEdit.setAlignment(Qt.AlignHCenter)
            main_window.dialog_inform.exec()
            return None

        self.progress_message.emit("품번을 구글 영국에서 검색하여 새 출연자 추가 작업을 진행합니다.")
        file_no = -1  # Current working file number
        file_no_total = 0
        file_no_newly_changed = 0
        main_window.request_no = 0  # The number of the Internet search tries.
        prod_code = ""
        prod_code_prev = ""  # To check if the prod_code has been searched on the internet
        file_base_name_new = ""

        # Get total number of files and set max row count of table object
        for (path, subfolders, files) in os.walk(main_window.label_work_path.text()):
            file_no_total += len(files)
        main_window.tableWidget.setRowCount(file_no_total)
        ...
        for (path, subfolders, files) in os.walk(main_window.label_work_path.text()):
            for file_name in files:
                ...
                file_base_name, ext = os.path.splitext(file_name)
                if ext[1:].lower() not in JAV_prod_code.formats_video_sub:
                    main_window.tableWidget.setItem(file_no, 0, QTableWidgetItem("건너 뛰기"))
                    main_window.tableWidget.setItem(file_no, 1, QTableWidgetItem("비디오 또는 자막 파일이 아님."))
                    main_window.tableWidget.setItem(file_no, 2, QTableWidgetItem(file_name))
                    self.progress_message.emit(f"{file_no}번 파일 처리.")
                    continue
                if file_base_name.upper().find("FC2") >= 0:
                    main_window.tableWidget.setItem(file_no, 0, QTableWidgetItem("건너 뛰기"))
                    main_window.tableWidget.setItem(file_no, 1, QTableWidgetItem("FC2 품번은 ABDBS에 등록되지 않음."))
                    main_window.tableWidget.setItem(file_no, 2, QTableWidgetItem(file_name))
                    self.progress_message.emit(f"{file_no}번 파일 처리.")
                    continue
                if len(file_base_name) <= 4:
                    main_window.tableWidget.setItem(file_no, 0, QTableWidgetItem("건너 뛰기"))
                    main_window.tableWidget.setItem(file_no, 1, QTableWidgetItem("파일 이름이 4자리 이하임."))
                    main_window.tableWidget.setItem(file_no, 2, QTableWidgetItem(file_name))
                    self.progress_message.emit(f"{file_no}번 파일 처리.")
                    continue
                ...
                try:
                    ...
                    main_window.tableWidget.setItem(file_no, 0, QTableWidgetItem("건너 뛰기"))
                    main_window.tableWidget.setItem(file_no, 1, QTableWidgetItem("파일 이름에서 품번을 찾을 수 없음."))
                    main_window.tableWidget.setItem(file_no, 2, QTableWidgetItem(file_name))
                    self.progress_message.emit(f"{file_no}번 파일 처리.")
                    continue
                except IndexError:
                    main_window.tableWidget.setItem(file_no, 0, QTableWidgetItem("프로그램 오류"))
                    main_window.tableWidget.setItem(file_no, 1, QTableWidgetItem(
                        "인덱스 오류. 파일 이름을 개발자에게 알려 주시면 프로그램을 수정하겠습니다."))
                    main_window.tableWidget.setItem(file_no, 2, QTableWidgetItem(file_name))
                    self.progress_message.emit(f"{file_no}번 파일 처리.")
                    continue
                ...
                if file_base_name.find(main_window.comboBox_sep_new.currentText()) >= 0:
                    main_window.tableWidget.setItem(file_no, 0, QTableWidgetItem("건너 뛰기"))
                    main_window.tableWidget.setItem(file_no, 1, QTableWidgetItem("이미 출연자 구분자 있음."))
                    main_window.tableWidget.setItem(file_no, 2, QTableWidgetItem(file_name))
                    self.progress_message.emit(f"{file_no}번 파일 처리.")
                    continue
                ...
                if prod_code != prod_code_prev:
                    ...
                    if main_window.request_failure_counter >= 7:
                        main_window.dialog_inform.textEdit.setText(
                            f"품번 인터넷 검색 실패가 7회 이상 반복되고 있습니다. "
                        ...
                        self.progress_message.emit(
                            ...
                            + f"구글 검색: {str(main_window.request_no)}번"
                        )
                        return None
                    ...
                    if warn_text_location > 0:
                        main_window.dialog_inform.textEdit.setText(
                            f"웹 검색 오류: '일상적이지 않은 트래픽 발견', 구글 차단 알림 위치: {str(warn_text_location)} \n"
                            + f"구글 검색 수: {str(main_window.request_no)}, 현재 작업 파일: {file_name}\n"
                        ...
                        main_window.request_failure_counter += 1
                        return None

                    web_title = soup.find("h3", "LC20lb MBeuO DKV0Md")
                    if web_title is None:
                        main_window.tableWidget.setItem(file_no, 0, QTableWidgetItem("건너 뛰기"))
                        main_window.tableWidget.setItem(file_no, 1, QTableWidgetItem("품번 구글 검색 실패"))
                        main_window.tableWidget.setItem(file_no, 2, QTableWidgetItem(file_name))
                    ...
                    if len(web_prod_code) <= 4:
                        main_window.tableWidget.setItem(file_no, 0, QTableWidgetItem("건너 뛰기"))
                        main_window.tableWidget.setItem(file_no, 1, QTableWidgetItem("품번 검색 실패"))
                        main_window.tableWidget.setItem(file_no, 2, QTableWidgetItem(file_name))
                    ...
                    if prod_code != web_prod_code:
                        ...
                        main_window.tableWidget.setItem(file_no, 0, QTableWidgetItem("건너 뛰기"))
                        main_window.tableWidget.setItem(file_no, 1, QTableWidgetItem("정확히 일치하는 품번 검색 실패"))
                        main_window.tableWidget.setItem(file_no, 2, QTableWidgetItem(file_name))

                        self.progress_message.emit(f"{file_no}번 파일 처리.")
                        main_window.request_failure_counter = 0
                        continue
                ...
                file_base_name_new = add_star_name(
                    ...
                    main_window.comboBox_position_new.currentText(),
                    main_window.comboBox_sep_new.currentText()
                )
                full_path_prev = os.path.join(path, file_name)
                full_path_new = os.path.join(path, file_base_name_new + ext)
                os.rename(full_path_prev, full_path_new)

                main_window.tableWidget.setItem(file_no, 0, QTableWidgetItem("성공"))
                main_window.tableWidget.setItem(file_no, 1, QTableWidgetItem("출연자 추가"))
                main_window.tableWidget.setItem(file_no, 2, QTableWidgetItem(file_name))
                main_window.tableWidget.setItem(file_no, 3, QTableWidgetItem(file_base_name_new + ext))
                self.progress_message.emit(f"{file_no}번 파일 처리.")
